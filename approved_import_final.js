...
const rawText = `- Stephen Manmara, manmaraste444@gmail.com
...
const emails = [...new Set([...rawText.matchAll(emailRegex)].map((m) => m[0].trim().toLowerCase()))].filter(Boolean);

const db = new sqlite3.Database('./database.sqlite');

console.log('EMAIL_COUNT=' + emails.length);
console.log('FIRST_EMAIL=' + emails[0]);

db.serialize(() => {
  db.run('DELETE FROM approved_students', (err) => {
    if (err) {
      console.error('DELETE_ERR=' + err.message);
      db.close();
      process.exit(1);
    }
    ...
    stmt.finalize(() => {
      db.get('SELECT COUNT(*) AS total FROM approved_students', (err2, row) => {
        if (err2) {
          console.error('COUNT_ERR=' + err2.message);
          db.close();
          process.exit(1);
        }

        console.log('APPROVED_COUNT=' + row.total);
        db.get('SELECT email FROM approved_students WHERE LOWER(email) = LOWER(?)', ['manmaraste444@gmail.com'], (err3, row3) => {
          console.log('MANMARA_CHECK=' + (row3 ? row3.email : 'NOT_FOUND'));
          db.close();
        });
      });
    });
